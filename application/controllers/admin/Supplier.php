...
class Supplier extends CI_Controller
{
    ...
    public function simpan()
    {
        ...
        if ($validasi->run()) { // menjalankan validasi form
            ...
            if ($insert) {
                echo '<script>alert("Data Berhasil Di Simpan");window.location.href = "' . site_url("admin/supplier") . '";</script>';
            } else {
                echo '<script>alert("Data Gagal Disimpan");window.location.href = "' . site_url("admin/supplier/simpan") . '";</script>';
            }
        }

        $data["list"] = $this->supplier_model->getList();
        $this->load->view("admin/tambah_supplier", $data); // menampilkan halaman tambah supplier
    }

    public function ubah($id_supplier = null) // menentukan parameter id_supplier menjadi null / kosong
    {
        if (!isset($id_supplier))
            redirect('admin/supplier'); //logika jika id_supplier yang dipanggil masih belum ada / kosong
        ...
        if ($validasi->run()) { // menjalankan validasi form
            ...
            if ($update) {
                echo '<script>alert("Data Berhasil Di Ubah");window.location.href = "' . site_url("admin/supplier") . '";</script>';
            } else {
                echo '<script>alert("Data Gagal Di Ubah");window.location.href = "' . site_url("admin/supplier/ubah") . '";</script>';
        ...
        $this->load->view("admin/ubah_supplier", $data); // memanggil data supplier ke halaman ubah supplier berdasarkan id_supplier
    }
    ...
    public function hapus($id_supplier = null) // menentukan parameter id_supplier menjadi null / kosong
    {
        ...
        if ($supplier->delete($id_supplier)) { // menghapus data berdasarkan id_supplier di db
            redirect('admin/supplier'); //menampilkan halaman supplier
